...
function createInjectionState(composable, options) {
  ...
  const defaultValue =
    options == null ? void 0 : options.defaultValue
...
function watchPausable(source, cb, options = {}) {
  ...
  const { eventFilter, pause, resume, isActive } =
    pausableFilter(filter, { initialState })
...
function syncRef(left, right, ...[options]) {
  ...
  const watchers = []
  const transformLTR =
    ('ltr' in transform && transform.ltr) || ((v) => v)
  const transformRTL =
    ('rtl' in transform && transform.rtl) || ((v) => v)
...
function isArrayIncludesOptions(obj) {
  return (
    isObject(obj) && containsProp(obj, 'formIndex', 'comparator')
  )
}
...
function formatOrdinal(num) {
  ...
  const v = num % 100
  return (
    num + (suffixes[(v - 20) % 10] || suffixes[v] || suffixes[0])
  )
}
...
function unrefElement(elRef) {
  ...
  return (_a = plain == null ? void 0 : plain.$el) != null
    ? _a
    : plain
}
function useEventListener(...args) {
  ...
  const firstParamTargets = computed(() => {
    const test = toArray(toValue(args[0])).filter(
      (e) => e != null
    )
    return test.every((e) => typeof e !== 'string')
      ? test
      : void 0
  })
  const stopWatch = watchImmediate(
    () => {
      ...
      return [
        ...
        toArray(
          toValue(firstParamTargets.value ? args[1] : args[0])
        ),
        toArray(
          unref(firstParamTargets.value ? args[2] : args[1])
        ),
...
function useMutationObserver(target, callback, options = {}) {
  const { window: window2 = defaultWindow, ...mutationOptions } =
    options
  let observer
  ...
  const element = customRef((track, trigger) => {
    ...
    return {
      get() {
        ...
        track()
        return (_b =
          (_a = instance == null ? void 0 : instance.proxy) ==
          null
            ? void 0
            : _a.$refs[key]) != null
          ? _b
...
function useActiveElement(options = {}) {
  ...
  const getDeepActiveElement = () => {
    ...
    if (deep) {
      while (element == null ? void 0 : element.shadowRoot)
        element =
          (_a2 =
            element == null ? void 0 : element.shadowRoot) ==
          null
            ? void 0
            : _a2.activeElement
    }
  ...
  if (triggerOnRemoval) {
    onElementRemoval(activeElement, trigger, {
      document: document2,
    })
  }
  trigger()
...
function useBreakpoints(breakpoints, options = {}) {
  ...
  function match(query, size) {
    ...
    return window2.matchMedia(`(${query}-width: ${size})`)
      .matches
  }
...
function useCycleList(list, options) {
  ...
  const index = computed({
    get() {
      ...
      if (index2 < 0)
        index2 =
          (_a =
            options == null ? void 0 : options.fallbackIndex) !=
          null
            ? _a
            : 0
      return index2
  ...
  function getInitialValue() {
    var _a, _b
    return (_b = toValue(
      (_a = options == null ? void 0 : options.initialValue) !=
        null
        ? _a
        : toValue(list)[0]
    )) != null
...
function useRefHistory(source, options = {}) {
  ...
  const { ignoreUpdates, ignorePrevAsyncUpdates, stop } =
...
function useDevicesList(options = {}) {
  ...
  async function ensurePermissions() {
    const deviceName = constraints.video
      ? 'camera'
      : 'microphone'
...
function useDisplayMedia(options = {}) {
  ...
  const isSupported = useSupported(() => {
    var _a2
    return (_a2 =
      navigator2 == null ? void 0 : navigator2.mediaDevices) ==
      null
      ? void 0
      : _a2.getDisplayMedia
  })
...
function useResizeObserver(target, callback, options = {}) {
  const { window: window2 = defaultWindow, ...observerOptions } =
    options
  let observer
  const isSupported = useSupported(
...
function useEventBus(key) {
  ...
  function on(listener) {
    var _a
    const listeners =
      events.get(key) || /* @__PURE__ */ new Set()
...
function useFileDialog(options = {}) {
  ...
  const { on: onChange, trigger: changeTrigger } =
    createEventHook()
  const { on: onCancel, trigger: cancelTrigger } =
    createEventHook()
  let input
  if (document2) {
...
function useFileSystemAccess(options = {}) {
  const { window: _window = defaultWindow, dataType = 'Text' } =
    options
...
function timeRangeToArray(timeRanges) {
  ...
  for (let i = 0; i < timeRanges.length; ++i)
    ranges = [
      ...ranges,
      [timeRanges.start(i), timeRanges.end(i)],
    ]
  return ranges
}
...
function useMediaControls(target, options = {}) {
  ...
  const { ignoreUpdates: ignoreCurrentTimeUpdates } =
...
function useOffsetPagination(options) {
  ...
  const currentPageSize = useClamp(
    pageSize,
    1,
    Number.POSITIVE_INFINITY
  )
...
function usePointerSwipe(target, options = {}) {
  ...
  tryOnMounted(() => {
    ...
    if (disableTextSelect) {
      ;(_d =
        (_c = targetRef.value) == null ? void 0 : _c.style) ==
      null
        ? void 0
        : _d.setProperty('-webkit-user-select', 'none')
      ;(_f =
        (_e = targetRef.value) == null ? void 0 : _e.style) ==
      null
        ? void 0
        : _f.setProperty('-ms-user-select', 'none')
      ;(_h =
        (_g = targetRef.value) == null ? void 0 : _g.style) ==
      null
        ? void 0
...
function useSorted(...args) {
  ...
  if (!dirty)
    return computed(() =>
      sortFn([...toValue(source)], compareFn)
    )
  watchEffect(() => {
...
function useSpeechRecognition(options = {}) {
  ...
  const SpeechRecognition =
    window2 &&
    (window2.SpeechRecognition ||
      window2.webkitSpeechRecognition)
...
function useTextSelection(options = {}) {
  ...
  const ranges = computed(() =>
    selection.value
      ? getRangesFromSelection(selection.value)
      : []
  )
...
function tryRequestAnimationFrame(window2 = defaultWindow, fn) {
  if (
    window2 &&
    typeof window2.requestAnimationFrame === 'function'
  ) {
...
function useTextareaAutosize(options = {}) {
  ...
  const textarea = toRef2(
    options == null ? void 0 : options.element
  )
  const input = toRef2(
...
function useHorizontalVirtualList(options, list) {
  ...
  const { state, source, currentList, size, containerRef } =
    resources
  ...
  if (isSupported.value) {
    useEventListener(
      ...
      () => {
        ...
        requestedType.value =
          (_b =
            (_a = sentinel.value) == null ? void 0 : _a.type) !=
          null
            ? _b
            : false
      },
...
function useWebNotification(options = {}) {
  ...
  const { on: onClick, trigger: clickTrigger } =
    createEventHook()
  const { on: onShow, trigger: showTrigger } = createEventHook()
  const { on: onError, trigger: errorTrigger } =
    createEventHook()
  const { on: onClose, trigger: closeTrigger } =
    createEventHook()
