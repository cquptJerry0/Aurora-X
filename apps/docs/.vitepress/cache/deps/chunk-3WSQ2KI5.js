...
function refreshComputed(computed3) {
  ...
  if (
    !computed3.isSSR &&
    computed3.flags & 128 &&
    ((!computed3.deps && !computed3._dirty) ||
      !isDirty(computed3))
  ) {
    return
  }
  ...
  try {
    ...
    if (
      dep.version === 0 ||
      hasChanged(value, computed3._value)
    ) {
      computed3.flags |= 128
...
function withDirectives(vnode, directives) {
  ...
  const instance = getComponentPublicInstance(
...
function propHasMismatch(el, key, clientValue, vnode, instance) {
  ...
  if (key === 'class') {
    ...
    if (
      !isSetEqual(toClassSet(actual || ''), toClassSet(expected))
    ) {
      mismatchType = 2
      mismatchKey = `class`
    }
...
function resolveComponent(name, maybeSelfReference) {
  return (
    resolveAsset(COMPONENTS, name, true, maybeSelfReference) ||
    name
  )
}
...
function resolveDynamicComponent(component) {
  if (isString(component)) {
    return (
      resolveAsset(COMPONENTS, component, false) || component
    )
  } else {
...
function getContext() {
  ...
  return (
    i.setupContext || (i.setupContext = createSetupContext(i))
  )
}
  ...
  const setupRenderEffect = (
    ...
    const componentUpdateFn = () => {
      if (!instance.isMounted) {
        ...
        if (
          (vnodeHook = next.props && next.props.onVnodeUpdated)
        ) {
          queuePostRenderEffect(
            () =>
              invokeVNodeHook(vnodeHook, parent, next, vnode),
...
function emit(instance, event, ...rawArgs) {
  ...
  if (!handler && isModelListener2) {
    handler =
      props[(handlerName = toHandlerKey(hyphenate(event)))]
  }
  if (handler) {
...
function addTransitionClass(el, cls) {
  ...
  ;(el[vtcKey] || (el[vtcKey] = /* @__PURE__ */ new Set())).add(
    cls
  )
}
...
function hasCSSTransform(el, root, moveClass) {
  ...
  if (_vtc) {
    _vtc.forEach((cls) => {
      cls
        .split(/\s+/)
        .forEach((c) => c && clone.classList.remove(c))
    })
  }
  moveClass
    .split(/\s+/)
...
vue/dist/vue.runtime.esm-bundler.js:
  ...
  **)
*/
//# sourceMappingURL=chunk-3WSQ2KI5.js.map
